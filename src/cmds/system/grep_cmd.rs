...
pub fn run(
    ...
    // Reconstruct the full commands so `rtk gain --history` preserves the
    // pattern, path, and flags the user invoked (instead of a bare "rtk grep").
    ...
    if has_format_flag(extra_args) {
        ...
        timer.track_passthrough(&original_cmd, &format!("{} (passthrough)", rtk_cmd));
